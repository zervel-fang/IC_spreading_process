...
void Print_value() ;
float Calculate_std(int *arr, int len, float average_value) ;
int main(int argn , char **argv)
{	// argv[4]:表示选取第几次模拟的结果 
	srand(time(NULL)) ;
	int tmp = 0 , count =0 ;
	float th1 = 0.0 , th2 = 0.0 , sum =0.0 ;
...
{	FILE *fin ;
	...
	char f2[5] = ".txt" ;
	char *file_path1 ;
	file_path1 = (char *)malloc((strlen(argv[1])+strlen(f1)+strlen(f2)+1)*sizeof(char)) ;
	sprintf(file_path1,"%s%s%s",f1,argv[1], f2)  ;
	...
	{	we_shoud_stop = 0  ;												   // for the node to infected its neighbor.				
		for(int i = 0 ; i < N ; i++)
		{
			if(IndexTag[i]==1&&Chance_to_Infect[i]==0) //if node1 was infected, then , it will infect its neighbor. 
			{
					...
					Chance_to_Infect[i] = 1 ;//该节点在下一轮不在影响其邻居节点。 
					we_shoud_stop++ ;
			}	
		}
		if(we_shoud_stop==0) 
...
void Output_Empirical_Real_pij(char **argv)
{
	...
	char f2[5] = ".txt" ;
	char *ff ; 
	ff = (char*)malloc((strlen(f1)+strlen(f2)+strlen(argv[1])+strlen(argv[4])+strlen(argv[3])+3)*sizeof(char)) ;
	sprintf(ff,"%s%s_%s_%s%s",f1,argv[1],argv[3],argv[4], f2) ;
	printf("%s\n",ff) ;
	fout = fopen(ff,"w")  ;
...
void Output_Empirical_Fstar_Gstar(char **argv)
{
	...
	char f2[5] = ".txt" ;
	char *ff ,*ff1; 
	ff = (char*)malloc((strlen(f1)+strlen(f2)+strlen(argv[3])+strlen(argv[4])+strlen(argv[1])+3)*sizeof(char)) ;
	ff1 = (char*)malloc((strlen(f3)+strlen(f2)+strlen(argv[3])+strlen(argv[4])+strlen(argv[1])+3)*sizeof(char)) ;
	sprintf(ff,"%s%s_%s_%s%s",f1,argv[1],argv[3],argv[4], f2) ;
	sprintf(ff1,"%s%s_%s_%s%s",f3,argv[1],argv[3], argv[4], f2) ; //文件格式 file_name_100.txt ; 
	printf("fstar is:%s\n",ff) ;
	printf("gstar is:%s\n",ff1) ;
	fout = fopen(ff,"w")  ;
	fout1 = fopen(ff1,"w")  ;
